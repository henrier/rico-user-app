...
void main() {
  group('CategoryProvider Tests', () {
    ...
    test('should select third category and update fourth level categories', () async {
      final notifier = container.read(categoryProvider.notifier);
      
      // 等待初始化完成
      await Future.delayed(const Duration(milliseconds: 600));
      
      final initialState = container.read(categoryProvider);
      ...
      final updatedState = container.read(categoryProvider);
      
      expect(updatedState.selectedThirdCategory?.name, 'MEGA');
      expect(updatedState.fourthLevelCategories.isNotEmpty, true);
      expect(
        updatedState.fourthLevelCategories.any((cat) => cat.name == 'Journey Together'),
        true,
      );
    });
    ...
    test('should not change selection if same category is selected', () async {
      ...
      expect(updatedState.fourthLevelCategories.length, initialFourthLevel.length);
    });
  });
}
